...
async function Handler(req, res) {
    ...
    try {
        ...
        if (admin !== null) {
            return res.json((new response_1.RKError('Người dùng này đang là quản trị viên')).toJSON());
        }
        await Admin_1.default.create(adminCond);
    }
    catch (err) {
        return res.json((new response_1.RKError('Đã có lỗi xảy ra', err)).toJSON());
    }
    return res.json((new response_1.RKSuccess('Nâng quyền quản trị thành công')).toJSON());
}
