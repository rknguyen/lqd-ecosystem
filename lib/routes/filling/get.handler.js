...
async function Handler(req, res) {
    ...
    for (let fill of filling) {
        ...
        const user = await Information_1.default.findOne({ userId: ownerID });
        const docs = await Document_1.default.findOne({ _id: documentID });
        fill.ownerName = user ? user.name : null;
        fill.docName = docs ? docs.name : null;
        fillingList.push(fill);
    }
    return res.json(new response_1.RKSuccess("Lấy bản ghi thành công", fillingList).toJSON());
}
